...
export function assertProvenanceStatement(statement, expected) {
  assert.equal(statement.predicateType, "https://slsa.dev/provenance/v1", "Expected SLSA v1 provenance");
  ...
  assert.deepEqual(
    ...
    [
      {
        name: `pkg:npm/${expected.name.replace(/^@/, "%40")}@${expected.version}`,
  ...
  assert.ok(
    definition.resolvedDependencies?.some(
      (dependency) =>
        dependency.uri === `git+https://github.com/${expected.repository}@${expected.ref}` &&
...
function derElement(bytes, offset = 0) {
  ...
  if (length & 0x80) {
    ...
    assert.ok(count > 0 && count <= 4 && offset + count <= bytes.length, "Invalid DER length");
...
export function assertProvenanceCertificate(rawBytes, expected) {
  ...
  const extensionContainers = derChildren(tbs.value).filter((entry) => entry.tag === 0xa3);
  assert.equal(extensionContainers.length, 1, "Expected one certificate extension container");
  ...
  for (const [suffix, expectedValue] of [
    ...
    const bytes = values.get(`2b0601040183bf3001${suffix.toString(16).padStart(2, "0")}`);
    ...
    assert.equal(value.value.toString("utf8"), expectedValue, `Wrong Fulcio identity extension ${suffix}`);
  }
}

export function assertVerifiedProvenance(audit, expected) {
  assert.deepEqual(audit.invalid, [], "npm reported invalid signatures");
  assert.deepEqual(audit.missing, [], "npm reported missing registry signatures");
  const verified = audit.verified?.filter(
    (entry) => entry.name === expected.name && entry.version === expected.version,
  );
  assert.ok(verified?.length, "Expected package is absent from npm's verified results");
  const statements = [];
  for (const entry of verified) {
    ...
    assert.equal(provenance?.length, 1, "Expected exactly one cryptographically verified provenance bundle");
    ...
    assert.ok(rawBytes, "Provenance requires a verified Fulcio signing certificate");
